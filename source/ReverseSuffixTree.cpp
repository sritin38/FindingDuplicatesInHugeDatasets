...
struct ReverseSuffixTree {
    std::vector<Node> nodes;

    ReverseSuffixTree(const std::string& str) {
        nodes.push_back(Node{});
        std::string reversedStr = str;
        std::reverse(reversedStr.begin(), reversedStr.end()); // Reverse the input string
        for (size_t i = 0; i < reversedStr.length(); i++) {
            addSuffix(reversedStr.substr(i)); // Add reversed suffixes
        }
    }
...
// int main() {
//     ReverseSuffixTree("banana$").visualize();
// }
